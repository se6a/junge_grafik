...
async function cacheViews() {
    const views = await fs.readdirSync(`${__basedir}/client/html/b_views`);

    Server.cache.viewsByName = views.map((_view) =>
        _view.replace(".view.js", "")
    );
}
...
function staticFiles() {
    Server.use(
        Express.static(`${__basedir}/client`, {
            dotfiles: "ignore",
            index: false,
        })
    );
