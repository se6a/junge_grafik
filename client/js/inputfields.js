document.addEventListener("DOMContentLoaded", () => {
    // "input" elements might have cached value
    // If its a hidden input, it needs to be reflected on it representative field:
    maybeCachedSelect();
    maybeCachedCheckbox();
});

function maybeCachedCheckbox() {
    const checkboxInputs = [
        ...document.querySelectorAll(".Checkbox.formField"),
    ];

    checkboxInputs.forEach(($checkbox) => {
        const $input = $checkbox.querySelector("input");
        const $display = $checkbox.querySelector(".input.Checkbox");
        const isChecked = $input.checked;

        $display.classList.toggle("--checked", isChecked);
    });
}

function maybeCachedSelect() {
    const selectInputs = [...document.querySelectorAll(".Select.formField")];
    selectInputs.forEach(($select) => {
        const $input = $select.querySelector("input");
        const $display = $select.querySelector(".input.Select > span");
        const value = $input.value;

        if (!value) return;

        const $option = $select
            .querySelector(`.Select.options > li[data-id="${value}"]`)
...
function validateForm($submitButton, e) {
    ...
    const invalides = [...$fields]
        ...
        .filter((_valid) => _valid !== null && _valid !== undefined);
